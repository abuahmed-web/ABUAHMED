    ...
    <div class="d-flex" id="wrapper">
        ...
        <div class="d-flex flex-column flex-shrink-0 p-3 text-white bg-dark" id="sidebar-wrapper">
            <a class="d-flex align-items-center mb-md-0 me-md-auto text-white text-decoration-none" href="/">
                <span class="fs-4">Mou Activation</span>
            </a>
            <hr>
            ...
            <ul aria-labelledby="dropdownUser1" class="dropdown-menu dropdown-menu-dark text-small shadow">
                <li><a class="dropdown-item" href="javascript:g_log_out()">AbuAhmed</a></li>
                <li><hr class="dropdown-divider"></li>
                <li><a class="dropdown-item" href="javascript:g_log_out()">تسجيل الخروج</a></li>
            </ul>
            <hr>
...
function activate_new_entry(projectId, type) {
    ...
    if (domainOrId) {
        $.ajax({
            url: 'https://control-panel.abuahmed-web.com/activate_entry.php',
            method: 'POST',
...
function toggle_activation(id, button) {
    // إرسال طلب لتبديل حالة التفعيل للـ ID المحدد
    $.post('toggle_activation.php', { id: id }, function(response) {
        try {
            var data = typeof response === 'string' ? JSON.parse(response) : response;
            
            if (data.success) {
                // تغيير شكل الزر بناءً على الحالة الجديدة
...
<!-- رابط CSS لـ Toastify -->
<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/toastify-js/src/toastify.min.css">

<!-- رابط JavaScript لـ Toastify -->
